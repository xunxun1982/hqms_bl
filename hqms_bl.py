...
def download_data():
    """下载数据并保存到Excel"""
    data = "pageNum=1&pageSize=500000000000&uploadstatus=0&a48="
    headers = {
        ...
        if os.path.exists(file_path):
            ...
            while attempts < max_retries:
                try:
                    with open(file_path, 'rb') as file:
                        file_content = file.read()
                    upload_url = f"https://blzk3.hqms.org.cn/blzk/diecasefile/add?indexid={row['id']}"   #三级、二级、民营按需 blzk3为民营，blzk为三级，blzk2为二级
                    files = {
                        'file': (os.path.basename(file_path), file_content, 'application/pdf')
                    }
                    # 发送文档上传请求
                    response = session.post(upload_url, files=files, timeout=timeout_seconds)
                    print(f"文档 {file_path} 上传结果: {response.status_code}")
                    break  # 成功上传，退出重试循环
                except requests.exceptions.Timeout:
                    attempts += 1
                    print(f"上传超时，正在重试... ({attempts}/{max_retries})")
                    sleep(5)  # 等待一段时间后重试
